...
class WolframAlphaQueryResult:
  ...
  def __getattr__(self, key):
    return scanbranches(self.tree, key)
  ...
  def Answer(self):
    if self.NumPods() < 2:
      ...
      return "".join(Subpod(Pod(self.Pods()[1]).Subpods()[0]).Plaintext())
...
class Pod:
  ...
  def __getattr__(self, key):
    return scanbranches(self.pod, key)

  def __str__(self):
    pp = PrettyPrinter()
    return pp.pformat(self.pod)
  ...
  def Plaintext(self):
    return scanbranches(self.pod, 'plaintext')
...
class Assumption:
  ...
  def __getattr__(self, key):
    return scanbranches(self.assumption, key)
